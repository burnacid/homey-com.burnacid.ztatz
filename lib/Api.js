...
class Api {
    ...
    async getWatermeter(apiVersion) {
        this.command = '/api/'+apiVersion+'/watermeter/hour?limit=1&json=object';
    ...
    async getWaterDay(apiVersion) {
        this.command = '/api/'+apiVersion+'/watermeter/day?json=object&limit=1';
    ...
    isToday(date){
        let d = new Date()
        let Today = new Date(d.toLocaleString('en-US', { timeZone: this.tz }))
